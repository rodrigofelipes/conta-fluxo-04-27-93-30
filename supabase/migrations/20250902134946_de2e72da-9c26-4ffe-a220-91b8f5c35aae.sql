-- Add collaborators to agenda
ALTER TABLE public.agenda ADD COLUMN IF NOT EXISTS collaborators_ids uuid[] DEFAULT '{}';
ALTER TABLE public.agenda ADD COLUMN IF NOT EXISTS agenda_type text DEFAULT 'compartilhada' CHECK (agenda_type IN ('pessoal', 'compartilhada'));
ALTER TABLE public.agenda ADD COLUMN IF NOT EXISTS visibility text DEFAULT 'team' CHECK (visibility IN ('private', 'team', 'public'));
...
-- Add transaction type classification to client_financials
ALTER TABLE public.client_financials ADD COLUMN IF NOT EXISTS transaction_category text DEFAULT 'other' CHECK (transaction_category IN ('receivable', 'payable', 'project', 'fixed_expense', 'variable_expense'));
ALTER TABLE public.client_financials ADD COLUMN IF NOT EXISTS recurrence_type text CHECK (recurrence_type IN ('none', 'monthly', 'quarterly', 'yearly'));
...
-- Add bank account reference to client_financials
